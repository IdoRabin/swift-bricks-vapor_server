...
final class BrickLayers : CodableHashable, BasicDescable {
    ...
    private var _cachedMNUID : MNUID? = nil
    var mnUID : MNUID? {
        guard let uid = self.id else {
            return nil
        }
        guard let cached = _cachedMNUID else {
            let res = MNUID(uidV5: uid, type: MNUIDTypes.doclayers)
            _cachedMNUID = res
            return res
        }
...
extension BrickLayers : BrickLayersContainer {
    ...
    func indexOfLayer(layer: BrickLayer) -> Int? {
        guard let mnUIDToFind = layer.mnUID as? LayerUID else {
            return nil
        }
        return indexesOfLayers(mnUIDs: [mnUIDToFind]).first?.value
    }
    
    func findLayersNearestTo(mnUIDs:[LayerUID], minGrow:UInt = 0, maxGrow:UInt = 2)->[BrickLayer] {
        ...
        let byIndex = self.layersByOrderIndex { layer in
            if let buid = layer.mnUID as? LayerUID {
                ...
                for sign in [-1, +1] {
                    ...
                    for index in indexes {
                        ...
                        if idx >= 0 && idx < self.count {
                            ...
                            if let layer = layer,let layerMNUID = layer.mnUID as? LayerUID, !mnUIDs.contains(layerMNUID) {
                                result.append(layer)
                                wasFound = true
                            }
    ...
    private func operateOnLayers(byMNUIDs mnUIDs:[LayerUID], block:([BrickLayer])->LayersResult)->LayersResult {
    ...
    func findLayers(mnUIDs: [LayerUID]) -> LayersResult {
        return self.findLayers { layer in
            if let layerMNUID = layer.mnUID as? LayerUID {
    ...
    func setLayersVisibility(mnUIDs: [LayerUID], newVisibilityState:BrickLayer.Visiblity) -> LayersResult {
    ...
    func setLayersAccess(mnUIDs: [LayerUID], newAccessState:BrickLayer.Access) -> LayersResult {
        self.operateOnLayers(byMNUIDs: mnUIDs) { layers in
            var changes = 0
            for layer in layers {
    ...
    func setLayersSelected(mnUIDs: [LayerUID], selectionState:BrickLayer.Selection) -> LayersResult {
        self.operateOnLayers(byMNUIDs: mnUIDs) { layers in
            var changesCount = 0
            for layer in layers {
    ...
    func selectLayers(mnUIDs:[LayerUID], deseletAllOthers:Bool = false)->LayersResult {
        ...
        case .success(let layers):
            ...
            if deseletAllOthers {
                let deselectIds : Set<LayerUID> = Set(orderedLayers.mnUIDs).subtracting(mnUIDs)
...
//    }
    ...
    var mnUIDs : [LayerUID] {
        return self.compactMap { layer in
            return layer.mnUID as? LayerUID
        }
    }
    ...
    func first(mnUID:LayerUID)->BrickLayer? {
        return self.filter({ layer in
            layer.mnUID == mnUID
        }).first
    }
    
    func filter(mnUIDs:[LayerUID])->[BrickLayer] {
        return self.filter({ layer in
            mnUIDs.contains(mnUID:layer.mnUID as? LayerUID)
        })
    }
    ...
    func excluding(mnUIDs:[LayerUID])->[BrickLayer] {
        return self.filter({ layer in
            return !mnUIDs.contains(mnUID:layer.mnUID as? LayerUID)
        })
    }
    
    func contains(allMNUIDs mnUIDs:[LayerUID])->Bool {
        return self.filter(mnUIDs: mnUIDs).count == mnUIDs.count
    }

    func contains(anyOfMNUIDs mnUIDs:[LayerUID])->Bool {
