...
//        }
    ...
    func willBoot(_ application: Vapor.Application) throws {
        let launchCount = (settings?.stats.launchCount ?? 1) + 1
        try self.permissions?.willBoot(application)
        dlog?.info("> Vapor app will Boot Nr.#\(launchCount)")
    }
    ...
    func didBoot(_ application: Vapor.Application) throws {
        ...
        let launchCount = (settings?.stats.launchCount ?? 1)
        isBooting = false
        
        dlog?.success("Vapor app did Boot: [\(Bundle.main.bundleName ?? "BServer") v\(Bundle.main.fullVersion)] run Nr.#\(launchCount) -[\(self.environment)]-")
    ...
    func shutdown(_ application: Vapor.Application) {
        ...
        */
        ...
        let launchCount = (settings?.stats.launchCount ?? 1)
        dlog?.info("> Vapor app will shutdown: [\(Bundle.main.bundleName ?? "BServer") v\(Bundle.main.fullVersion)] run Nr.#\(launchCount)")
