...
fileprivate class AppConfigurator {
    ...
    """
    //    PostgresDatabase().withConnection { connection in
    ...
    fileprivate func configureDBInstance(_ app: Application) async throws {
        ...
        let configFactory : DatabaseConfigurationFactory = .postgres(hostname: "localhost",
                                                                     username: "vapor",
                                                                     password: "vapor",
                                                                     database: dbName)
        app.databases.use(configFactory, as: .psql, isDefault: true)
        
        app.logger.info("migrateDB setting DB named: [\(dbName)] iana port: \(PostgresConfiguration.ianaPortNumber)") // iana == 5432
...
//            AppServer.shared.users.db = app.db
//            AppServer.shared.permissions?.db = app.db
            ...
            let desc = ""
            // Uncomment:   = error.description
            // Uncomment: dlog?.warning("configure failed: \(desc)")
