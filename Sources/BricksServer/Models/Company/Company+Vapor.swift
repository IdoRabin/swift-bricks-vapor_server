...
fileprivate let dlog : DSLogger? = DLog.forClass("Company")

// Extensions requiring no implementations
extension Company : Content {}
extension Company : Authenticatable {}
extension Company : AsyncResponseEncodable {
    
}

/// Keys for data stored in the User of Requests:
struct CompanyStorageKey : ReqStorageKey {
    typealias Value = Company
}

// MARK: Validatable User
extension Company: Validatable {
